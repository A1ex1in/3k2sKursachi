...
class Authorization(QMainWindow):
    ...
    def login(self):
        ...
        if role == 'cliet':
            ...
            QMessageBox.warning(self, parent='Error', QWidget='Invalid role')
...
class UserWind(QMainWindow):
    ...
    def qSchet(self):
        data = self.DataFromDB("SELECT * FROM schet")
        dialog = TableDialog(self.conn)
        dialog.set_data(data)
        dialog.exec_()

    def qInfo(self):
        data = self.DataFromDB("SELECT * FROM client")
        dialog = TableDialog(self.conn)
        dialog.set_data(data)
        dialog.exec_()

    def qOper(self):
        data = self.DataFromDB("SELECT * FROM operations_schet")
        dialog = TableDialog(self.conn)
        dialog.set_data(data)
        dialog.exec_()
...
class ManajerWind(QMainWindow):
    def __init__(self, connection):
        ...
        self.ui.CB_Table.addItems(name_table)
        self.ui.CB_Querry.addItems(name_query)
